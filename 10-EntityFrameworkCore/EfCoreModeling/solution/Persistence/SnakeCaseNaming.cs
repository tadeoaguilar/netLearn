...
public static class SnakeCaseNaming
{
    public static void UseSnakeCaseNames(this ModelBuilder modelBuilder)
    {
        foreach (var entity in modelBuilder.Model.GetEntityTypes())
        {
            ...
            // An owned type sharing its owner's table (e.g. Book.Price) has a
            // shadow property that is BOTH its primary key and the foreign
            // key back to the owner -- it always holds the exact same value
            // as the owner's own primary key, and is never materialized as
            // its own column. EF Core's model validator specifically trusts
            // that link as long as its column name is left at its
            // convention-assigned default; explicitly renaming it (even to
            // an equivalent string) makes the validator treat it as an
            // independent column and reject it as incompatible with the
            // owner's primary key. So it is the one property left alone here.
